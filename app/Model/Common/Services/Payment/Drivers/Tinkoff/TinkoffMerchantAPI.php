...
// Load official class from tinkoff php SDK
...
class TinkoffMerchantAPI
{
    ...
    {
        return $data['Token'] === $this->_genToken($data);
    }

    private function _genToken(array $data): string
    {
        if (isset($data['Token'])) {
            unset($data['Token']);
        }
        ...
        $token = join('', array_map(function ($value) {
